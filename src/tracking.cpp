...
void tracking::loadparams()
{
    ros::NodeHandle n_p("~");
    // Load Server Parameters
    n_p.param<std::string>("modelname", modelname, "reemc.urdf");
    rd = new COM_EKF::robotDyn(modelname, false);
...
void tracking::loadJointKFparams()
{
    ros::NodeHandle n_p("~");
...
void tracking::loadIMUEKFparams()
{
    ros::NodeHandle n_p("~");
...
void tracking::loadCoMEKFparams()
{
    ros::NodeHandle n_p("~");
...
bool tracking::connect(const ros::NodeHandle nh)
{
    ROS_INFO_STREAM("COM_EKF Initializing...");
    ...
    advertise();
    //ros::NodeHandle np("~")
    //dynamic_recfg_ = boost::make_shared< dynamic_reconfigure::Server<COM_EKF::VarianceControlConfig> >(np);
    //dynamic_reconfigure::Server<COM_EKF::VarianceControlConfig>::CallbackType cb = boost::bind(&tracking::reconfigureCB, this, _1, _2);
    // dynamic_recfg_->setCallback(cb);
    is_connected_ = true;
    ros::Duration(1.0).sleep();
    ROS_INFO_STREAM("COM_EKF Initialized");
    return true;
}
...
void tracking::estimateWithIMUEKF()
{
...
void tracking::estimateWithCoMEKF()
{
    if (nipmEKF->firstrun)
    {
...
void tracking::computeKinTFs()
{
    ...
    if (!kinematicsInitialized)
    {
        ...
        dr = new COM_EKF::deadReckoning(Twl.translation(), Twr.translation(), Twl.linear(), Twr.linear(),
    ...
    if (firstContact)
    {
        cd = new COM_EKF::ContactDetection();
        if (useGEM)
        {
...
void tracking::computeGlobalCOP(Affine3d Twl_, Affine3d Twr_)
{
...
void tracking::advertise()
{

    SupportPose_pub = n.advertise<geometry_msgs::PoseStamped>("COM_EKF/support/pose", 1000);

    baseIMU_pub = n.advertise<sensor_msgs::Imu>("COM_EKF/base/acc", 1000);

    LLegOdom_pub = n.advertise<nav_msgs::Odometry>("COM_EKF/LLeg/odom", 1000);

    RLegOdom_pub = n.advertise<nav_msgs::Odometry>("COM_EKF/RLeg/odom", 1000);

    SupportLegId_pub = n.advertise<std_msgs::String>("COM_EKF/support/leg", 1000);

    baseOdom_pub = n.advertise<nav_msgs::Odometry>("COM_EKF/base/odom", 1000);

    CoMLegOdom_pub = n.advertise<nav_msgs::Odometry>("COM_EKF/CoM/leg_odom", 1000);

    if (computeJointVelocity)
        joint_pub = n.advertise<sensor_msgs::JointState>("COM_EKF/joint_states", 1000);

    if (useCoMEKF)
    {
        CoMOdom_pub = n.advertise<nav_msgs::Odometry>("COM_EKF/CoM/odom", 1000);
        ExternalWrench_pub = n.advertise<geometry_msgs::WrenchStamped>("COM_EKF/CoM/wrench", 1000);
        COP_pub = n.advertise<geometry_msgs::PointStamped>("COM_EKF/COP", 1000);
    }
    RLegWrench_pub = n.advertise<geometry_msgs::WrenchStamped>("COM_EKF/RLeg/wrench", 1000);

    LLegWrench_pub = n.advertise<geometry_msgs::WrenchStamped>("COM_EKF/LLeg/wrench", 1000);

    legOdom_pub = n.advertise<nav_msgs::Odometry>("COM_EKF/base/leg_odom", 1000);

    if (ground_truth)
    {
        ground_truth_com_pub = n.advertise<nav_msgs::Odometry>("COM_EKF/ground_truth/CoM/odom", 1000);

        ground_truth_odom_pub = n.advertise<nav_msgs::Odometry>("COM_EKF/ground_truth/base/odom", 1000);
    }

    if (debug_mode)
    {
        rel_LLegPose_pub = n.advertise<geometry_msgs::PoseStamped>("COM_EKF/rel_LLeg/pose", 1000);

        rel_RLegPose_pub = n.advertise<geometry_msgs::PoseStamped>("COM_EKF/rel_RLeg/pose", 1000);

        rel_CoMPose_pub = n.advertise<geometry_msgs::PoseStamped>("COM_EKF/rel_CoM/pose", 1000);
    }

    if (comp_with)
        comp_odom0_pub = n.advertise<nav_msgs::Odometry>("COM_EKF/comp/base/odom0", 1000);
}

void tracking::subscribeToJointState()
{

    joint_state_sub = n.subscribe(joint_state_topic, 1000, &tracking::joint_stateCb, this);
    firstJointStates = true;
}

void tracking::joint_stateCb(const sensor_msgs::JointState::ConstPtr &msg)
{
    joint_data.push(*msg);
...
void tracking::joints(const sensor_msgs::JointState &msg)
{
...
void tracking::subscribeToOdom()
{

    odom_sub = n.subscribe(odom_topic, 1000, &tracking::odomCb, this);
    firstOdom = true;
}

void tracking::odomCb(const nav_msgs::Odometry::ConstPtr &msg)
{
    odom_msg = *msg;
    odom_inc = true;
    if (firstOdom)
...
void tracking::subscribeToGroundTruth()
{
    ground_truth_odom_sub = n.subscribe(ground_truth_odom_topic, 1000, &tracking::ground_truth_odomCb, this);
    firstGT = true;
}
void tracking::ground_truth_odomCb(const nav_msgs::Odometry::ConstPtr &msg)
{
...
void tracking::subscribeToGroundTruthCoM()
{
    ground_truth_com_sub = n.subscribe(ground_truth_com_topic, 1000, &tracking::ground_truth_comCb, this);
    firstGTCoM = true;
}
void tracking::ground_truth_comCb(const nav_msgs::Odometry::ConstPtr &msg)
{
    if (kinematicsInitialized)
    {
...
void tracking::subscribeToCompOdom()
{

    compodom0_sub = n.subscribe(comp_with_odom0_topic, 1000, &tracking::compodom0Cb, this);
    firstCO = true;
}

void tracking::compodom0Cb(const nav_msgs::Odometry::ConstPtr &msg)
{
    if (kinematicsInitialized)
    {
        comp_odom0_msg = *msg;
...
void tracking::subscribeToIMU()
{
    imu_sub = n.subscribe(imu_topic, 1000, &tracking::imuCb, this);
}
void tracking::imuCb(const sensor_msgs::Imu::ConstPtr &msg)
{
...
void tracking::subscribeToFSR()
{
    //Left Foot Wrench
    lfsr_sub = n.subscribe(lfsr_topic, 1000, &tracking::lfsrCb, this);
    //Right Foot Wrench
    rfsr_sub = n.subscribe(rfsr_topic, 1000, &tracking::rfsrCb, this);
}

void tracking::lfsrCb(const geometry_msgs::WrenchStamped::ConstPtr &msg)
{
...
void tracking::LLeg_FT(const geometry_msgs::WrenchStamped &msg)
{
...
void tracking::rfsrCb(const geometry_msgs::WrenchStamped::ConstPtr &msg)
{
...
void tracking::RLeg_FT(const geometry_msgs::WrenchStamped &msg)
{
...
void tracking::publishBodyEstimates()
{
    if (useIMUEKF)
    {
...
void tracking::publishSupportEstimates()
{
...
void tracking::publishLegEstimates()
{
...
void tracking::publishJointEstimates()
{
...
void tracking::publishCoMEstimates()
{
...
void tracking::outputPublishThread()
{
