...
void DVE::init()
{
    ...
    std::cout << "Joint velocity filter Initialized Successfully" << std::endl;
}


std::unique_ptr<Eigen::Matrix<double, 15, 15>> DVE::computeTrans(const std::unique_ptr<Eigen::Matrix<double, 15, 1>>& x_,
...
void DVE::euler(const std::unique_ptr<Vector3d>& omega_,const std::unique_ptr<Vector3d>& f_)
{
...
std::unique_ptr<Matrix<double, 15, 1>> DVE::computeDiscreteDyn(const std::unique_ptr<Matrix<double, 15, 1>>& x_,
                                                          const std::unique_ptr<Quaterniond>& qib, 
...
void DVE::predict(const std::unique_ptr<Vector3d>& omega_, const std::unique_ptr<Vector3d>& f_)
{
    (*omega_old) = (*omega);
    (*omega) = (*omega_);
    (*f) = (*f_);
...
void DVE::updateWithTwistRotation(const std::unique_ptr<Vector3d>& y,const std::unique_ptr<Eigen::Quaterniond>& qy)
{
